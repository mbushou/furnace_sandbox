...
class App(object):
    ...
    def __init__(self, ctx):
        ...
        fe.event_register({'event_type': fe.BE,
                           'callback': self.be_callback})
        fe.event_register({'event_type': fe.TIMER,
                           'time_value': 2.0,
    ...
    def memdump_callback(self, ctx):
        fe.log(f'memdump_callback: coff={self.offset}, cttl={self.ttl}, memsize={self.memsize}')
        ...
        if self.ttl > 0:
            ...
            fe.notify(json.dumps({'cmd': 'memdump', 'data': chunk_arr,
                                  'ix': self.ticks, }))#'hash': hashlib.sha256(res).hexdigest()}))
    ...
    def be_callback(self, ctx):
        ...
        if msg['cmd'] == 'memdump_cmd' and msg['data'] == 'go':
            if self.did >= 0:
                ...
                fe.notify(json.dumps({'cmd': 'error', 'data': 'memdump already running'}))
            else:
                fe.log(f'running memdump')
                fe.notify(json.dumps({'cmd': 'memdump_running', 'data': time.time()}))
                self.did = fe.event_register({'event_type': fe.TIMER,
                                            'time_value': 0.0,
