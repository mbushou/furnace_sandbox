...
static event_response_t linux_cb(drakvuf_t drakvuf, drakvuf_trap_info_t* info)
{
    ...
    printf("[SYSCALL %lu] in syscall plugin callback\n", s->calls);
    printf("vCPU:%" PRIu32 " CR3:0x%" PRIx64 ",%s %s:%" PRIi64" %s!%s\n",
           info->vcpu, info->regs->cr3, info->proc_data.name,
           USERIDSTR(drakvuf), info->proc_data.userid,
           info->trap->breakpoint.module, info->trap->name);
    s->output_callback(s->output_data, info);
    return 0;
...
syscalls::syscalls(drakvuf_t drakvuf, const void* config,
                   void* output_data,
                   void (*output_callback) (void* data, drakvuf_trap_info_t* info)
                  )
{
    printf("IN SYSCALL CONSTRUCTOR\n");
